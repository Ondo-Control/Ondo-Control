...
> **Was Fassung 0.145 ändert (24.9.2026):** Reine Fassungszahl-Anpassung nach Arbeitsregel M — `STAND.md` und der Backlog wurden für die letzte Nachbesserung zu Backlog-Punkt 86 geändert (`beta.html` v19.19.2: ein kritischer Checkpoint gilt nur noch mit abgeschlossenem IndexedDB-Schreiben; `save()` behält seinen `localStorage`-Rückfall). Blueprint selbst inhaltlich nicht geändert. Die Präzisierung steht in `Ondo-Core-Architektur.md`, Abschnitt 1e. Blueprint auf 0.145 gehoben.
...
*Stand im Code: Die frühere Aussage, die blinde „nimm das nächste Modell"-Logik existiere seit v19.7.4 nicht mehr, war unzutreffend. Vor Backlog-Punkt 88 setzte `geminiCall()` nach der Kontingentprüfung weiterhin jeden anderen Fehler faktisch auf „Modellproblem". Seit v19.20.0 gilt Arbeitsregel N auch technisch: Kontingent hält sofort an; nur ausdrücklich erkannte Überlastung darf automatisch zum nächsten Modell wechseln; jede andere Fehlerart liefert den sichtbaren Zustand `_unklar` und beendet den Aufruf ohne weiteren Versuch. Unverändert zulässig bleibt der bekannte Konfigurationsfall: fehlt für den vorhandenen Schlüssel überhaupt ein stärkeres Modell, wird mit Flash geprüft — das ist kein Fehler-Rückfall.*
...
**✅ Geschlossen — Beförderungskriterium (c):** Backlog-Punkt 5 ist seit 13.9.2026 entschieden — Ondo: „Wir haben genug Spiele!!!!", kein Blocker mehr.
