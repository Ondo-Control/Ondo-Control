# ONDO CONTROL — STAND
*Die aktuelle Wahrheit. Stand: 28.8.2026, Fassung 56, v19.8.6*
...
- **Sprachschlüssel: 229** in DE, FR und EN, maschinell abgeglichen und identisch (**selbst gezählt von `pruefe.py` Abschnitt 13, Stand 28.8.2026**). *Verlauf: die früher dokumentierten 184 waren nie geprüft; nachgezählt waren es 185, dann 193, dann 199, dann 201 (v19.7.8), dann 203 (v19.8.1). Die acht Schlüssel des Berichtigungsknopfes vom 13.8. (`korrT` bis `korrOk`) waren nirgends nachgetragen — 203 + 8 = 211. Punkt 44 bringt sechs weitere (`messT` bis `messBlock`) — 211 + 6 = 217. Backlog-Punkt 51 (gepaarter Vergleich) bringt elf weitere (`gepaart` bis `gepaartMehrdeutig`) — 217 + 11 = 228. Backlog-Punkt 64 bringt einen weiteren (`parkGrundInstabil`) — 228 + 1 = 229.* **Diese Zahl ist bei jeder Änderung an den Sprachschlüsseln in derselben Lieferung mitzuführen.**
...
**Nutzungslimit-Schätzung (Backlog-Punkt 56, 27.8.2026):** Anthropic veröffentlicht keine festen Zahlen; dieses Feld wird nur gelegentlich von Ondo per Bildschirmfoto kalibriert, keine Automatik, keine feste Häufigkeit. **Zuletzt gesehen: 28.8.2026, 21:43 Uhr** — aktuelle 5-Stunden-Sitzung 49 % verbraucht (Reset in 3 Std. 6 Min.), Wochenlimit „Alle Modelle" 47 % verbraucht (Reset Donnerstag 03:59 Uhr), Nutzungsguthaben weiterhin 0 Punkte, Funktion ausgeschaltet. *(Diese Ablesung ersetzt die vom 28.8.2026, 13:33 Uhr — 85 % / 33 % — vollständig, nicht ergänzend.)* Cloud-/Code-Sitzungen und Chat teilen sich dasselbe Kontingent (belegt über `support.claude.com`, 27.8.2026).

**⚠ Zwei verschiedene Zahlen, nicht verwechseln:** Die 5-Stunden-Sitzung setzt sich alle fünf Stunden zurück und sagt nichts über den Tag als Ganzes. Das Wochenlimit summiert dagegen über die ganze Woche seit dem Reset (Donnerstag 03:59 Uhr). **Laut Ondo liefen am 28.8. insgesamt DREI verschiedene 5-Stunden-Sitzungslimits nacheinander** (jedes für sich nach fünf Stunden zurückgesetzt) — die 49 % von 21:43 Uhr gehören bereits zum dritten dieser Fenster, nicht zum selben wie die 85 % von 13:33 Uhr. **Das Wochenlimit ist von 26 % am Ende des 27.8. (23:08 Uhr) auf 47 % um 21:43 Uhr gestiegen — ein Anstieg von 21 Prozentpunkten über den ganzen 28.8.**, verteilt auf alle heutigen Sitzungen zusammen (dieser Chat, unabhängige Ergebnis-Recherche, unabhängige Job-Recherche, mehrere Code-Sitzungen) — Einzelheiten in den Sitzungs-Selbstauskünften unten.
...
f) **Chat-Sitzung Chat 26 (Nachfolger von Chat 25/(d), aus der Übergabe in dieser Sitzung hervorgegangen), Sonnet 5/Mittel — Stand 21:43 Uhr:** rund 17 Prompts seit Sitzungsbeginn — **eine eigene, neue Sitzung, nicht dieselbe wie (d) fortgeführt.** Aufwand hoch bis sehr hoch: mehrere vollständige Abrufe von `STAND.md` und Backlog (drei verschiedene Commit-Stände), ein vollständiger Abruf von `CHRONIK-2026-08.md`, fünf ausgelesene Webarchive, rund ein Dutzend Bildschirmfoto-Analysen. Kosten klar über dem Richtwert vom 27.8. — **ausdrücklich keine genaue Zahl genannt** (Scheinpräzision).
g) **Diese Code-Sitzung, Stand 21:43 Uhr — seit (e) bis einschliesslich dieses Auftrags:** 12 weitere Prompts (Übergabemappe- und Abnahme-Erstellung an Chat 26, Leeds-Park-Status-Berichtigung, Beteiligtenkreis-Aktualisierung, Backlog-Punkt 51 gebaut, Backlog-Punkt 64 gebaut samt Celje/Sabah-Markierung, Nachfrage Modellversion bei `refRoh`, dieser Nutzungsdaten-Auftrag). Aufwand sehr hoch — deutlich über „Mittel": drei vollständige Plan-Modus-Bauzyklen mit Codeuntersuchung in `beta.html`, Edits, `node --check`, Python-Rechenproben und `pruefe.py`-Läufen (mindestens acht Läufe insgesamt), zwei Websuchen, sechs Commits mit Push, mehrere vollständig neu geschriebene Planungsdateien (je 100–200 Zeilen). Kosten grob geschätzt **eher im Bereich 400–700 KB** bewegten Textes für diese 12 Prompts — deutlich über dem Richtwert von 23,8 KB/Prompt (das entspräche nur rund 286 KB bei „Mittel"), keine genaue Zahl (Scheinpräzision). Nicht unabhängig gegengeprüft.

> **🔴 BERICHTIGT, noch am selben Abend (28.8., nach Ondos Rückfrage).** Hier stand bis eben: (d) sei „zuletzt aktualisiert 21:43 Uhr" auf „17 Prompts" — als wäre die 21:43-Uhr-Auskunft (heute Punkt f) dieselbe Sitzung, nur später fortgeführt (15→17). **Das war falsch, aus blosser Textähnlichkeit hergeleitet statt belegt (Arbeitsregel H).** Die Übergabe Chat 25 → Chat 26 ist in dieser Sitzung selbst dokumentiert (Übergabemappe und Abnahme, weiter oben) — (d) ist Chat 25, bis zur Übergabe, (f) ist Chat 26, eine eigene neue Sitzung danach. Beide stehen jetzt getrennt.

**Richtwert überprüft, Stand 21:43 Uhr (Teil 3 des Nutzungsdaten-Auftrags) — 23,8 KB/Prompt bleibt bestehen, kein Nachfolgewert eingetragen.** Gegen alle sieben Selbstauskünfte (a)–(g) geprüft: Die Streuung ist zu gross, um daraus einen einzelnen neuen „Mittel"-Wert seriös abzuleiten. Auffällig: **Keine einzige der sieben Auskünfte stuft sich selbst als „Mittel" ein** — sie liegen an den beiden Enden, entweder „sehr gering" ((a), (c), unter 1,5 KB/Prompt) oder „hoch bis sehr hoch" ((b), (d), (e), (f), (g), grob zwischen 20 und über 70 KB/Prompt, mit Ausreissern bis über 100 KB/Prompt bei einzelnen websuchlastigen Prompts). Ein Durchschnitt über diese Extreme hinweg würde nicht „Mittel" neu kalibrieren, sondern unvergleichbare Aufwandsstufen vermischen — genau der Fehler, den eine einzelne erzwungene Zahl hier begehen würde. **Der bestehende Richtwert (23,8 KB/Prompt, Stand 27.8.2026) bleibt deshalb unverändert stehen**, weiterhin ausdrücklich als grober Anhaltspunkt, keine Formel. Für spätere Kalibrierung festgehalten, nicht heute umgesetzt: Ein einzelner Richtwert bildet die tatsächliche Streuung schlecht ab — separate Anhaltspunkte je Aufwandsstufe (gering/mittel/hoch) wären ehrlicher, sind aber eine grössere Änderung an der Methodik und keine Aufgabe dieses Auftrags.
